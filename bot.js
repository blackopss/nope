...
const adminprefix = "!";
const developers = ['431150885549113344'];
...
client.on('message', message => {
  ...
  if (message.content.startsWith(adminprefix + 'ply')) {
    client.user.setGame(argresult);
      message.channel.send(`**nope   ${argresult}**`)
  } else 
  ...
  client.user.setActivity(argresult, {type:'WATCHING'});
      message.channel.send(`**who?   ${argresult}**`)
  } else 
  ...
  client.user.setUsername(argresult).then
      message.channel.send(`go to hell**${argresult}** `)
} else
...
client.on("message", async msg => {
  ...
  if (msg.content.toLowerCase().startsWith(prefix + "mute")) {
      ...
      if(!mentions && username.size == 0) return msg.channel.send('I cant find the user');
...
client.on('message', message => {
    ...
    if (msg.content === 'الوان') {
      if (msg.channel.id !== "476154997000896522") return;
...
if (message.channel.id !== "476154997000896522") return;
...
client.on("guildMemberAdd", (member) => {
    ...
    var guild;
    while (!guild)
        guild = client.guilds.find("name", "Elite Store Soon")
    guild.fetchInvites().then((data) => {
        data.forEach((Invite, key, map) => {
            var Inv = Invite.code;
            if (dat[Inv])
 ...
 Link Of The Invite --> https://discord.gg/${Invite.code} 
 Nubmer --> ${member.guild.memberCount}  **`)            
 }
            dat[Inv] = Invite.uses;
        })
    })
